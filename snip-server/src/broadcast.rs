...
impl Broadcaster {
    ...
    pub async fn broadcast(&self, op: String, data: String) {
        // TODO:
        // - CHECK THE MATCH
        let clients = self.inner.lock().clients.clone();
        let msg: String = serde_json::to_string(&Message { op: op, data: data }).unwrap();
        let send_futures = clients
            .iter()
            .map(|client| client.send(SseData::new(msg.as_str())));
